 ...
 */
/* jshint -W062 */
/* global GainlabParams, WOW, AOS, WVC, WolfCore, Cookies, Event, objectFitImages, WVCBigText, WVCParams, GainlabYTVideoBg, Vimeo */

var GainlabUi = (function ($) {
    ...
    return {
        ...
        init: function () {
            ...
            this.isMobile = GainlabParams.isMobile;
			this.mobileScreenBreakpoint =
				GainlabParams.mobileScreenBreakpoint;
			this.tabletScreenBreakpoint =
				GainlabParams.tabletScreenBreakpoint;
			this.notebookScreenBreakpoint =
				GainlabParams.notebookScreenBreakpoint;
			this.desktopScreenBreakpoint =
				GainlabParams.desktopScreenBreakpoint;
			this.desktopBigScreenBreakpoint =
				GainlabParams.desktopBigScreenBreakpoint;
		...
		setClasses: function () {
			...
			if (
				...
				!GainlabParams.forceAnimationMobile
			) {
		...
		loadingAnimation: function () {
			if (!GainlabParams.defaultPageLoadingAnimation) {
				return false;
			}
			...
			if ($.isFunction($.fn.stick_in_parent)) {
				if (
					...
					$(".work-info-container").stick_in_parent({
						offset_top: parseInt(
							GainlabParams.portfolioSidebarOffsetTop,
							10
						),
					});
				}
		...
		mobileMenuBreakPoint: function ( hard ) {
			...
			var hard = hard || false,
				...
				breakpoint = GainlabParams.breakPoint;
			...
			if (breakpoint > winWidth) {
				...
				$(window).trigger("gainlab_breakpoint");
		...
		subMenuDirection: function () {
			var $this,
				subMenuWidth = parseInt(GainlabParams.subMenuWidth, 10),
				bleed = 8;
		...
		stickyMenu: function (scrollTop) {
			var scrollPoint,
				menuOffset = parseInt(GainlabParams.menuOffset, 10);

			scrollTop = scrollTop || 0;

			scrollPoint =
				parseInt(GainlabParams.stickyMenuScrollPoint, 10) ||
				$("body").offset().top;
			...
			if (menuOffset) {
				if ("100%" === GainlabParams.menuOffset) {
					scrollPoint =
						$(window).height() -
						parseInt(GainlabParams.desktopMenuHeight, 10);
				} else {
					scrollPoint =
						menuOffset -
						parseInt(GainlabParams.desktopMenuHeight, 10);
			...
			if ("soft" === GainlabParams.stickyMenuType) {
			...
			} else if ("hard" === GainlabParams.stickyMenuType) {
				if (scrollPoint < scrollTop) {
		...
		parallax: function () {
			var smallScreen =
				...
				GainlabParams.parallaxNoSmallScreen;

			if (!smallScreen) {
				$(".parallax").jarallax({
					noAndroid: GainlabParams.parallaxNoAndroid,
					noIos: GainlabParams.parallaxNoIos,
				});
			}
		},
		...
		toggleMenu: function () {
			...
			$(document).on("click", ".toggle-mobile-menu", function (event) {
				...
				$(window).trigger("gainlab_mobile_menu_toggle_button_click", [
					$(this),
				]);
			...
			$(document).on("click", ".toggle-side-panel", function (event) {
				event.preventDefault();
				$(window).trigger("gainlab_side_panel_toggle_button_click", [
					$(this),
				]);
			...
			$(document).on("click", ".toggle-offcanvas-menu", function (event) {
				event.preventDefault();
				$( window ).trigger("gainlab_offcanvas_menu_toggle_button_click", [
					$(this),
				]);
			...
			$(document).on("click", ".toggle-overlay-menu", function (event) {
				...
				$(window).trigger(
					"gainlab_overlay_menu_toggle_button_click",
					[$(this)]
				);
			...
			$(document).on("click", ".toggle-lateral-menu", function (event) {
				event.preventDefault();
				$(window).trigger(
					"gainlab_lateral_menu_toggle_button_click",
					[$(this)]
				);
		...
		toggleSearchForm: function () {
			$(document).on('click', '.toggle-search', function ( event ) {
				event.preventDefault();
				$(window).trigger('gainlab_searchform_toggle');
		...
		flexSlider: function () {
			if ($.isFunction($.flexslider)) {
				...
				$(".entry-slider").flexslider({
					animation: GainlabParams.entrySliderAnimation,
					slideshow: true,
					slideshowSpeed: 4000,
				});
			}
		...
		lightbox: function () {
			...
			if ("fancybox" === GainlabParams.lightbox) {
				$(selectors).fancybox(GainlabParams.fancyboxSettings);
				...
				$(".gallery-quickview, .wvc-gallery-quickview, .wolf-core-gallery-quickview")
					...
					.on("click", function ( event ) {
						...
						params = GainlabParams.fancyboxSettings;
				...
				$(".woocommerce-product-gallery__trigger")
					...
					.on("click", function ( event ) {
						...
						$.fancybox.open(
							quickviewData,
							GainlabParams.fancyboxSettings
						);
						return false;
					});
				...
				$(".lightbox-video, .wp-block-image.video-box a").fancybox({
					beforeLoad: function () {
						GainlabUi.pausePlayers();
					},

					afterLoad: function () {
						GainlabUi.lightboxVideoAfterLoad();
					},

					afterClose: function () {
						GainlabUi.restartVideoBackgrounds();
					},
				});
			}
		},
		...
		lightboxVideoAfterLoad: function () {
			var $iframe = $(".fancybox-iframe").contents(),
				...
				accentColor = GainlabParams.accentColor;
			...
			if ($video.length) {
				...
				$head.append(
					$("<link/>", {
						rel: "stylesheet",
						href: GainlabParams.mediaelementLegacyCssUri,
						type: "text/css",
					})
				);
				$head.append(
					$("<link/>", {
						rel: "stylesheet",
						href: GainlabParams.mediaelementCssUri,
						type: "text/css",
					})
				);
				...
				$head.append(
					$("<link/>", {
						rel: "stylesheet",
						href: GainlabParams.fancyboxMediaelementCssUri,
						type: "text/css",
					})
				);
		...
		scrollToMainContent: function () {
			...
			$("html, body")
				...
				.animate(
					...
					parseInt(GainlabParams.smoothScrollSpeed, 10),
					GainlabParams.smoothScrollEase,
					function () {
						if ( "" !== hash) {
		...
		smoothScroll: function (href) {
			var scrollOffset = this.getToolBarOffset() - 5 - parseInt( GainlabParams.smoothScrollOffsetAdjust ),
				hash;
			...
			if (-1 !== href.indexOf("#")) {
				...
				if ($("#" + hash).length) {
					if (
						"hard" === GainlabParams.stickyMenuType &&
						...
						scrollOffset += parseFloat(
							GainlabParams.stickyMenuHeight,
							10
						);
					}
		...
		wowAnimate: function () {
			...
			if ("undefined" !== typeof WOW) {
				wowAnimate = new WOW({
					boxClass: GainlabParams.WOWBoxClass,
					offset: GainlabParams.WOWAnimationOffset,
		...
		liveSearch: function () {
			...
			$(".live-search-form").each(function () {
				var $formContainer = $(this),
					...
					timer = null,
					$resultContainer,
					action = "gainlab_ajax_live_search",
					result;

				if ($form.hasClass("woocommerce-product-search")) {
					action = "gainlab_ajax_woocommerce_live_search";
				}
				...
				searchInput.on("keyup", function (event) {
					...
					if (8 === event.keyCode || 46 === event.keyCode) {
						...
						timer = setTimeout(function () {
							...
							$.post(GainlabParams.ajaxUrl, data, function (
								response
							) {
								if ("" !== response) {
			...
			if (!this.isMobile) {
				...
				$(document).on("added_to_cart", function (
					...
					if (
						...
						$tipspan.attr(
							"title",
							GainlabParams.l10n.addedToCart
						);
		...
		setInternalLinkClass: function () {
			var siteURL = GainlabParams.siteUrl,
				$internalLinks,
				regEx = "";

			$.each(GainlabParams.allowedMimeTypes, function (
				index,
				value
			) {
				regEx += "|" + value;
			...
			if (GainlabParams.isWooCommerce) {
				/*
				...
				if (
					...
					this.untrailingSlashit(
						GainlabParams.WooCommerceCartUrl
					)
				) {
					$(
						"a[href^='" +
				...
				if (
					this.untrailingSlashit(siteURL) !==
					this.untrailingSlashit(
						GainlabParams.WooCommerceAccountUrl
					)
				) {
					$(
						"a[href^='" +
				...
				if (
					this.untrailingSlashit(siteURL) !==
					this.untrailingSlashit(
						GainlabParams.WooCommerceCheckoutUrl
					)
				) {
					$(
						"a[href^='" +
		...
		transitionCosmetic: function () {
			if (!GainlabParams.defaultPageTransitionAnimation) {
				return false;
			}

			var _this = this;

			if (GainlabParams.isAjaxNav) {
				return;
			}
			...
			$(document).on("click", ".internal-link:not(.disabled)", function (
				...
				if (!event.ctrlKey) {
					...
					if (GainlabParams.hasLoadingOverlay) {
						$(".loading-overlay").one(
		...
		videoThumbnailPlayOnHover: function () {
			...
			$("iframe.youtube-bg", itemsContainer).each(function () {
				if (
					"undefined" !== typeof GainlabYTVideoBg &&
					GainlabYTVideoBg.players[
				...
				if ($iframe.length) {
					if ($iframe.hasClass("youtube-bg")) {
						$(this).on("mouseenter", function () {
							if (
								"undefined" !== typeof GainlabYTVideoBg &&
								GainlabYTVideoBg.players[YTPlayerId]
							) {
								GainlabYTVideoBg.players[
									YTPlayerId
								].playVideo();
		...
		navPlayer: function () {
			$(document).on("click", ".nav-play-button", function () {
				...
				var $btn = $(this),
					...
					audioId = $audio.attr("id"),
					audio = document.getElementById(audioId),
					playText = GainlabParams.l10n.playText,
					pauseText = GainlabParams.l10n.pauseText;
		...
		revSliderErrorMessage : function() {
			if ( GainlabParams.isWolfCore && $( '.rs_error_message_box' ).length ) {
				$( '.rs_error_message_content' ).after( '<div class="rs_error_message_content">' + GainlabParams.l10n.missingRevsliderHelpMessage + '</div>' );
			}
		},
		...
		pageLoad: function () {
			...
			delay = GainlabParams.pageTransitionDelayAfter;
			...
			}, parseInt(GainlabParams.pageLoadedDelay, 10));
		},
	};
})(jQuery);
...
(function ($) {
	...
	if ("undefined" !== typeof WVC) {
		 ...
		 */
		WVC.lightbox = GainlabUi.lightbox;

		/**
		 * Overwrite WVC video opener
		 */
		WVC.videoOpener = GainlabUi.videoOpener;

		/**
		 * Overwrite toolbar offset calculation
		 */
		WVC.getToolBarOffset = GainlabUi.getToolBarOffset;
	}
	...
	if ("undefined" !== typeof WolfCore) {
		 ...
		 * Overwrite WVC lightbox function with the theme function
		 */
		WolfCore.lightbox = GainlabUi.lightbox;

		/**
		 * Overwrite WolfCore video opener
		 */
		WolfCore.videoOpener = GainlabUi.videoOpener;

		/**
		 * Overwrite toolbar offset calculation
		 */
		WolfCore.getToolBarOffset = GainlabUi.getToolBarOffset;
	}
